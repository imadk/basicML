evaluate_svm_score <- function (all = FALSE){
	...
	if(!all){ #ident only
		...
		proj$pr <- predict(proj$ksvm, proj$testset[,c(proj$input)])
		
		sdata <- subset(proj$testset[,], select=c(proj$ident))
		
		write.csv(cbind(sdata, proj$pr), file=paste(ftest,"_score_svm_id.csv",sep=""), row.names=FALSE)
